...
def run_stock_research(
    *,
    ...
    return {
        ...
        "markdown_path": str(document_paths["zh"]),
        ...
        artifacts.append(
            {
                ...
                "markdown_path": primary_document_path,
    ...
    for item in artifacts:
        lines.extend(
            [
                ...
                f"- Report: `{item.get('primary_document_path', '') or item.get('zh_docx_path', '') or item.get('markdown_path', '')}`",
                "",
            ]
        )
    return "\n".join(lines)
...
def execute_email_command(
    *,
    ...
    if kind == "screen":
        ...
        body = render_email_screen_reply(theme=command["theme"], payload=artifact["payload"], markdown_path=artifact["primary_document_path"])
...
def render_email_screen_reply(*, theme: str, payload: dict[str, Any], markdown_path: str) -> str:
    ...
    lines.extend(["", "Desk action:", f"- Attached screening summary document: {markdown_path}"])
    return "\n".join(lines)
